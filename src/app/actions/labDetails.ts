...
// 🔥 UPDATED: Verify BOTH Email and Mobile OTP
export async function verifyOTP(email: any, emailOTP: any, mobileOTP: any) {
  try {
    ...
    // Check if both OTPs match (since we send same OTP to both channels)
    if (emailOTP !== mobileOTP) {
      console.log("❌ Email and Mobile OTPs don't match");
      return {
        success: false,
        error: "Email and Mobile OTPs must be the same",
      };
    }
    
    // Verify the OTP
    const isValid = await validateOTP(email, emailOTP);
    ...
    await removeOTP(email);
    console.log("✅ Both OTPs verified and removed");
